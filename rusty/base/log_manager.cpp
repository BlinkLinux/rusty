...
void LogManager::registerConsoleLog(LogLevel level) {
  ...
  this->p_->console_appender->setDetailsLevel(static_cast<Logger::LogLevel>(level));
  this->p_->console_appender->setFormat(this->p_->log_format);
  cuteLoggerInstance()->registerAppender(this->p_->console_appender);
}

void LogManager::registerFileLog(LogLevel level) {
  this->registerFileLog(level, kDefaultFileLimits);
}

void LogManager::registerFileLog(LogLevel level, int file_limit) {
  Q_ASSERT(this->p_ != nullptr);
  if (this->p_->file_appender != nullptr) {
    cuteLoggerInstance()->removeAppender(this->p_->file_appender);
  }
  this->p_->file_appender = new RollingFileAppender(this->p_->log_path);
  this->p_->file_appender->setFormat(this->p_->log_format);
  this->p_->file_appender->setLogFilesLimit(file_limit);
  this->p_->file_appender->setDetailsLevel(static_cast<Logger::LogLevel>(level));
